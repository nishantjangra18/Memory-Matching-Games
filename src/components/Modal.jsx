...
export default function Modal({ moves, time, onRestart, gameMode, player1Score, player2Score, player1Name, player2Name }) {
  ...
  const getScoreDisplay = () => {
    if (gameMode === "1v1") {
      return (
        <div className="space-y-4">
          <div className="flex justify-between items-center p-4 bg-gradient-to-r from-blue-500/20 to-purple-500/20 rounded-xl border border-blue-500/30">
            <span className="font-semibold text-lg">{player1Name}</span>
            <span className="text-2xl font-bold text-blue-300">{player1Score}</span>
          </div>
          <div className="flex justify-between items-center p-4 bg-gradient-to-r from-red-500/20 to-pink-500/20 rounded-xl border border-red-500/30">
    ...
    return (
      <div className="space-y-4">
        <div className="flex justify-between items-center p-4 bg-gradient-to-r from-green-500/20 to-blue-500/20 rounded-xl border border-green-500/30">
  ...
  return (
    ...
    >
      <motion.div 
        ...
        className="bg-gradient-to-br from-slate-800/95 to-slate-900/95 backdrop-blur-md p-8 sm:p-10 rounded-3xl shadow-2xl border border-white/20 max-w-md w-full"
      >
