...
export default function GameBoard({ difficulty, setMoves, setGameOver, gameStarted, gameMode, currentPlayer, onMatchComplete, onTurnSwitch }) {
  ...
  const getGridCols = () => {
    if (difficulty === "easy") return "grid-cols-3 sm:grid-cols-3 md:grid-cols-3";
    if (difficulty === "medium") return "grid-cols-4 sm:grid-cols-4 md:grid-cols-4";
    return "grid-cols-4 sm:grid-cols-6 md:grid-cols-6 lg:grid-cols-6";
  };
  ...
  const getCardSize = () => {
    ...
    if (difficulty === "medium") return "w-14 h-18 sm:w-16 sm:h-20 md:w-18 md:h-22";
    return "w-12 h-16 sm:w-14 sm:h-18 md:w-16 md:h-20";
  };
  ...
  return (
    <div className={`grid gap-2 sm:gap-3 ${getGridCols()} p-3 sm:p-4 bg-white/5 rounded-2xl border border-white/10 ${getContainerWidth()} mx-auto`}>
      {cards.map((card) => {
        ...
        return (
          ...
          >
            ...
            >
              ...
              <div
                className={`absolute inset-0 flex items-center justify-center rounded-lg text-lg sm:text-xl font-bold ${
                  isFlipped 
                    ? 'bg-slate-700' 
              ...
              {/* Card Front (Icon) */}
              <div
                className="absolute inset-0 flex items-center justify-center rounded-lg bg-indigo-500 shadow-lg shadow-indigo-500/30 text-lg sm:text-xl font-bold border border-white/20"
                style={{ 
                  backfaceVisibility: 'hidden',
